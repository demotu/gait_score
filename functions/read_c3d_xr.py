...
def get_events(fname, missing=np.nan, only_1cycle=True):
    ...
    if c3d.parameters().isGroup('EVENT'):
        ...
        if only_1cycle:
            evs = events
            # one cycle (if exists) as outer limits of events
            if 'RFS' in evs.keys() and len(evs['RFS']) > 1 and evs['1'][1] != 'RFS':
                evs['1'] = [evs['RFS'][1], 'RFS']
            elif 'LFS' in evs.keys() and len(evs['LFS']) > 1 and evs['1'][1] != 'LFS':
                evs['1'] = [evs['LFS'][1], 'LFS']
            events = evs
...
def plot(da, *args, var=None, sharey=False, size=2.5, aspect=1.5, show=True,
    ...
    plot_lines(axes=g.axs, evs=da.Time.attrs['events'], hline=hline,
               evline=evline)
    if title:
